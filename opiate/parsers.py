...
def group_samples(infile, ctl_names = CONTROL_NAMES, split_desc = lambda desc: desc.split()[0]):
    ...
    (`controls`,`sample_groups`) where `controls` is keyed by names in
    ctl_names (default is `opiate.CONTROL_NAMES`) and `sample_groups`
    is a dict of groups generated by applying function `split_desc` to
    each SAMPLE_desc. Dicts containing compound data in sample_groups
    are given an additional key 'sample_prep' returning an element of
    opiate.SAMPLE_PREP_LABELS corresponding to their position in the
    group *if* the group is of length 4.
    ...
    for label, grp in groupby(samples, lambda s: split_desc(s[0]['SAMPLE_desc'])):
        ...
        if len(this_group) == len(SAMPLE_PREP_LABELS):
            for sample_prep, sample in zip(SAMPLE_PREP_LABELS, this_group):
                for cmpnd in sample:
