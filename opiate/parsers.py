...
from __init__ import qafile, SAMPLE_ATTRS, CONTROL_NAMES
...
def group_samples(infile, std_names = CONTROL_NAMES, split_desc = lambda desc: desc.split()[0]):
    """
    Parse an XML-format file and return two OrderedDict objects:
    (`standards`,`sample_groups`) where `standards` is keyed by names
    in std_names (default is `opiate.CONTROL_NAMES`) and
